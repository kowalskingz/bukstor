...
class Pesan extends CI_Controller
{
	...
	public function index()
	{
		...
		{	
			echo '<script type="text/javascript">alert("Anda harus login terlebih dahulu..");window.location="home/login"</script>';
		}
		...
		if ($this->input->post('submit', TRUE) == 'Submit')
		{
			...
			if ($this->form_validation->run() == TRUE) 
			{
				...
				if ($get->num_rows() > 0) 
				{
					...
					$config['useragent'] = 'BUKSTOR';
					$config['protocol'] = 'smtp'; //protokol
					$config['mailtype'] = 'html'; //tipe mailnya
					$config['smtp_host'] = 'ssl://smtp.gmail.com'; //host
					$config['smtp_port'] = '465'; //port
					$config['smtp_timeout'] = '5';
					$config['smtp_user'] = 'kopibukancopy@gmail.com'; //isi dengan email gmail
					$config['smtp_pass'] = 'kopi1gelas'; //isi dengan password
					$config['crlf'] = "\r\n";
					$config['newline'] = "\r\n";
					...
					foreach ($this->cart->contents() as $carts)
					{
						$table .= '<tr>
									...
									<td>'.$carts['name'].'</td>
									<td>'.$carts['qty'].'</td>
									<td style="text-align:right">'.number_format($carts['subtotal'], 0, ',','.').'</td>';
					}

					$this->email->from('bukstortkt@gmail.com', "BUKSTOR");
                ...
                $data = array(
						...
						if ($this->app->insert('t_order', $data)) 
						{
							...
							echo '<script type="text/javascript">alert("Pesanan berhasil, Silahkan cek email anda untuk detail pembayaran..");
					...
					else
					{
						...
						// echo '<script type="text/javascript">alert("Pesanan gagal terkirim")</script>';
					}
				}
   ...
   public function city()
   {
    ...
    "key:  54cfabd41b2802f48b2144cf1d08e964"
