...
class Administrator extends CI_Controller 
{
	...
	public function index()
	{
		...
		$data['buku'] = $this->admin->count('t_buku');
		$data['transaksi'] = $this->admin->count_where('t_order', ['status' => 'proses']);
	...
	public function edit_profil()
	{
		...
		if ($this->input->post('submit', TRUE) == 'Submit') 
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{
					echo '<script type="text/javascript">alert("Password yang Anda masukkan salah");window.location.replace("'.base_url().'index.php/login/logout")</script>';
				}
	...
	function update_password()
	{
		...
		if ($this->input->post('submit', TRUE) == 'Submit') 
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{
					echo '<script type="text/javascript">alert("Password lama yang Anda masukkan salah");window.location.replace("'.base_url().'index.php/login/logout")</script>';
				}
...
// $2y$10$zezP0ym5H9Qzuu72F8KmSeqCa/OGvc97WjiJOIDJhiLBKqhSXJn/S